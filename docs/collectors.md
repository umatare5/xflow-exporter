...
- `flags` — the TCP control bits the flow's packets ORed together, rendered as names from the low bit up, the order every packet tool prints them in (`syn`, `syn,ack`, `fin,psh,ack`). Only TCP records feed it, and only those from a device that exported the field. A segment setting no bit is a NULL scan rather than a gap, so it keys `none` rather than being dropped.
