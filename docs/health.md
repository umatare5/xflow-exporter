...
**Correction Precedence**: A record takes the rate of the sampler it names, failing that its own domain's declaration, and failing that the one rate every declaration on the device agrees on. Where none answers, the counts are corrected by one and no `xflow_sampling_rate` series exists, as for a domain whose records name their own samplers. `xflow_sampling_unresolved_flows_total` separates the two, appearing once the device has declared, so a restart leaves it absent until the device re-announces.
