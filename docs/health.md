...
the table a size or an eviction count belongs to, named as its module is.
...
- `mapping` and `services` both name an application from a port and `mapping` runs first, so a record `mapping` names reaches `services` as `skipped`, while one only `services` names was already counted `unknown`.
...
each names a bound the wire cannot raise, and each counts attempts rather than the entities refused — [Bounded state](README.md#bounded-state) carries the budgets and what a refusal costs the records behind it.

**`xflow_enrichment_lookups_total`**

`unknown` is the ordinary reading for a source that covers part of the traffic, so read the split as coverage rather than as failure.

**`xflow_threat_entries`, `xflow_threat_skipped_lines` and the two reload counters**

`xflow_threat_reload_failures_total` rising is what says the set in force is older than the file on disk, a failed load deliberately keeping the previous set — [Threat lists](README.md#threat-lists) carries why.
